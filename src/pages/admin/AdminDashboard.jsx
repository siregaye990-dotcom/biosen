...
import { ordersApi, stockApi, visitsApi } from '../../lib/supabase'
import { STATUS_MAP, STOCK_KEYS } from '../../utils/products'
...
export default function AdminDashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setOrders(local.length ? local : SEED_ORDERS())
      setStock(SEED_STOCK)
      // Mock 7-day visits
      const days = ['Lun','Mar','Mer','Jeu','Ven','Sam','Dim']
      setVisits(days.map((d,i) => ({ date: d, count: [28,42,35,61,55,48,72][i] })))
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const handleAdvance = async (order) => {
    ...
    if (cur === 'delivered' || cur === 'cancelled') { toast('Statut final atteint'); return }
    const ni = STATUS_FLOW.indexOf(cur) + 1
    if (ni >= STATUS_FLOW.length) return
    const newStatus = STATUS_FLOW[ni]
    try {
      await ordersApi.updateStatus(order.id, newStatus)
    } catch {
      // Fallback: update localStorage
      const local = JSON.parse(localStorage.getItem('biosen_orders_fallback') || '[]')
      const updated = local.map(o => o.order_number === order.order_number ? { ...o, status: newStatus } : o)
      localStorage.setItem('biosen_orders_fallback', JSON.stringify(updated))
    }
    setOrders(prev => prev.map(o => o.id === order.id ? { ...o, status: newStatus } : o))
    toast.success(`→ ${STATUS_MAP[newStatus]?.label}`)
  }
  ...
  const handleCancel = async (order) => {
    ...
    try {
      ...
      const local = JSON.parse(localStorage.getItem('biosen_orders_fallback') || '[]')
      const updated = local.map(o => o.order_number === order.order_number ? { ...o, status: 'cancelled' } : o)
      localStorage.setItem('biosen_orders_fallback', JSON.stringify(updated))
    }
